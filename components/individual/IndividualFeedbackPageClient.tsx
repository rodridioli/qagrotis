...
export function IndividualFeedbackPageClient({
  ...
  // Reset campos when tipo changes (only for new feedbacks)
  React.useEffect(() => {
    if (isNew) setCampos({})
  }, [tipo, isNew])

  const userQuery = `?userId=${encodeURIComponent(evaluatedUserId)}`
  const listHref = `/individual/feedbacks${userQuery}`
          ...
          <div className="space-y-1.5">
            ...
            {!isNew && !isViewOnly && (
              <p className="text-xs text-text-secondary">
                O tipo não pode ser alterado após a criação do rascunho.
              </p>
            )}
          </div>
              ...
              <div key={field.key} className="space-y-1.5">
                ...
                {isViewOnly ? (
                  ...
                  <textarea
                    ...
                    disabled={busy != null}
                    rows={4}
                    className="w-full resize-y rounded-lg border border-border-default bg-surface-input px-3 py-2 text-sm text-text-primary placeholder:text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
                  />
                )}
              </div>
            ))}
