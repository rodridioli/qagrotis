...
import { Check, ChevronLeft, ChevronRight, Loader2, Pencil, Star } from "lucide-react"
...
import { listChapterRatings, createChapterRating } from "@/features/equipe/actions/equipe-chapters"
import type { EquipeChapterRatingEntry } from "@/features/equipe/lib/equipe-chapters-shared"
import { ChapterStarsSummary } from "@/features/equipe/components/ChapterStarsSummary"
        ...
        {view === "overview" ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <DialogFooter className="flex-col gap-2 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" className="gap-1.5" onClick={() => setView("overview")}>
                <ChevronLeft className="size-4 shrink-0" />
                Voltar
              </Button>
              <Button type="submit" className="gap-1.5" disabled={saving || isPreview} title={isPreview ? "Pré-visualização sem servidor." : undefined}>
                {saving
                  ? <><Loader2 className="size-4 shrink-0 animate-spin" />Salvando…</>
                  : <><Check className="size-4 shrink-0" />Avaliar</>
                }
              </Button>
            </DialogFooter>
          </form>
